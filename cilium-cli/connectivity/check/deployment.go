...
func (ct *ConnectivityTest) deployNamespace(ctx context.Context) error {
	for _, client := range ct.Clients() {
		if ct.params.ForceDeploy {
			...
			if err := ct.DeleteZtunnelTestEnv(ctx, client); err != nil {
				return err
			}
		}
...
func (ct *ConnectivityTest) deploy(ctx context.Context) error {
	...
	if ct.Features[features.Ztunnel].Enabled && ct.params.testEnabled("ztunnel-pod-to-pod-encryption") {
...
func (ct *ConnectivityTest) DeleteZtunnelTestEnv(ctx context.Context, client *k8s.Client) error {
	namespaces := []string{"cilium-test-ztunnel-enrolled-0", "cilium-test-ztunnel-enrolled-1", "cilium-test-ztunnel-unenrolled"}

	for _, ns := range namespaces {
		// Delete deployments
		for _, deploymentName := range []string{clientDeploymentName, echoSameNodeDeploymentName, echoOtherNodeDeploymentName} {
			_, err := client.GetDeployment(ctx, ns, deploymentName, metav1.GetOptions{})
			if err == nil {
				ct.Logf("🔥 [%s] Deleting %s deployment in %s ns...", client.ClusterName(), deploymentName, ns)
				_ = client.DeleteDeployment(ctx, ns, deploymentName, metav1.DeleteOptions{})
			}
		}
