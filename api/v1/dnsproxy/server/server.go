...
// Cell implements the standalone DNS proxy REST API server when provided
// the required request handlers.
var Cell = cell.Module(
	"standalone-dns-proxy-server",
	"standalone DNS proxy server",
...
// APICell provides the restapi.StandaloneDNSProxyAPI type, populated
...
type apiParams struct {
	...
	Middleware middleware.Builder `name:"standalone-dns-proxy-middleware" optional:"true"`

	DnsproxyGetHealthzHandler dnsproxy.GetHealthzHandler
	DnsproxyGetReadyzHandler  dnsproxy.GetReadyzHandler
}

func newAPI(p apiParams) *restapi.StandaloneDNSProxyAPI {
	api := restapi.NewStandaloneDNSProxyAPI(p.Spec.Document)

	// Construct the API from the provided handlers

	api.DnsproxyGetHealthzHandler = p.DnsproxyGetHealthzHandler
	api.DnsproxyGetReadyzHandler = p.DnsproxyGetReadyzHandler
...
type serverParams struct {
	...
	Logger     *slog.Logger
	Spec       *Spec
	API        *restapi.StandaloneDNSProxyAPI
}
...
type ServerConfig struct {
	EnableStandaloneDNSProxyServerAccess []string
}
...
var (
	...
	AdminEnableFlag = "enable-standalone-dns-proxy-server-access"
)

func (cfg ServerConfig) Flags(flags *pflag.FlagSet) {
	flags.StringSlice(AdminEnableFlag, cfg.EnableStandaloneDNSProxyServerAccess,
		"List of standalone DNS proxy APIs which are administratively enabled. Supports '*'.")
}

var SpecCell = cell.Module(
	"standalone-dns-proxy-spec",
	"standalone DNS proxy Specification",
...
func newSpec(cfg ServerConfig) (*Spec, error) {
	...
	deniedAPIs, err := api.AllowedFlagsToDeniedPaths(swaggerSpec, cfg.EnableStandaloneDNSProxyServerAccess)
	if err != nil {
...
// NewServer creates a new api standalone DNS proxy server but does not configure it
func NewServer(api *restapi.StandaloneDNSProxyAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
// Server for the standalone DNS proxy API
type Server struct {
	EnabledListeners []string
...
func (s *Server) SetAPI(api *restapi.StandaloneDNSProxyAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) GetAPI() *restapi.StandaloneDNSProxyAPI {
	return s.api
}
...
func (s *Server) Start(cell.HookContext) (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		s.wg.Add(1)
		s.Logf("Serving standalone DNS proxy at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := domainSocket.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving standalone DNS proxy at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		s.wg.Add(1)
		s.Logf("Serving standalone DNS proxy at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := httpServer.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving standalone DNS proxy at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		s.wg.Add(1)
		s.Logf("Serving standalone DNS proxy at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := httpsServer.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving standalone DNS proxy at https://%s", l.Addr())
