...
func (ops *EnrollmentReconciler) Start(ctx cell.HookContext) error {
	...
	go func() {
		...
		if ops.spireClient == nil {
			ops.logger.Info("SPIRE client is nil, skipping mTLS enrollment reconciler")
			return
		}
		...
		resyncCh := ops.spireClient.ResyncNeeded()
		for {
			...
			case <-watch:
			case <-resyncCh:
				ops.resyncAllEnrolledNamespaces()
			}
		}
	}()
	return nil
...
// resyncAllEnrolledNamespaces re-creates SPIRE entries for all service accounts
// in all enrolled namespaces. This is called after the SPIRE server reconnects
// (e.g. after a restart with ephemeral storage) to restore lost entries.
func (ops *EnrollmentReconciler) resyncAllEnrolledNamespaces() {
	txn := ops.db.ReadTxn()
	trustDomain := ops.spireClient.GetSpireTrustDomain()
	spireEntryConfig := ops.spireClient.GetSpireEntryConfig()
	pathFunc := spireEntryConfig.PathFunc
	...
	all := ops.enrolledNamespaceTable.All(txn)
	for ns := range all {
		sas := ops.serviceAccountTable.List(txn, ServiceAccountNamespaceIndex.Query(ns.Name))
		...
		err := ops.spireClient.InsertBatch(context.Background(), entries)
		if err != nil {
			ops.logger.Error("failed to resync CA entries after SPIRE reconnect",
				slog.String("namespace", ns.Name),
				slog.String("error", err.Error()))
		} else {
			ops.logger.Info("Resynced CA entries after SPIRE reconnect",
				slog.String("namespace", ns.Name),
				slog.Int("serviceaccounts", len(entries)))
		}
	}
}
