...
type Namespace struct {
	*slim_corev1.Namespace
	...
	Enrolled bool
	// PendingCACreation indicates if a CA creation request is pending for the namespace.
	PendingCACreation bool
	// PendingCADeletion indicates if a CA deletion request is pending for the namespace.
	PendingCADeletion bool
	Status            reconciler.Status // reconciliation status
}

// TableHeader implements statedb.TableWritable.
func (ns *Namespace) TableHeader() []string {
	return []string{"Name", "Enrolled for mTLS", "Pending CA Creation", "Pending CA Deletion", "Status"}
}

// TableRow implements statedb.TableWritable.
func (ns *Namespace) TableRow() []string {
	return []string{ns.Name, fmt.Sprintf("%t", ns.Enrolled), fmt.Sprintf("%t", ns.PendingCACreation), fmt.Sprintf("%t", ns.PendingCADeletion), ns.Status.String()}
}
...
func NewEnrolledNamespacesTable(jg job.Group, db *statedb.DB, cs client.Clientset, zcfg config.ZtunnelConfig) (statedb.RWTable[*Namespace], error) {
	...
	return k8s.ReflectorConfig[*Namespace]{
		...
		Transform: func(txn statedb.ReadTxn, obj any) (*Namespace, bool) {
			...
			if found && prevEntry.Enrolled && !enrolled {
				pendingCADeletion = true
			}
			return &Namespace{
				Namespace:         namespace,
				Enrolled:          enrolled,
				PendingCACreation: pendingCACreation,
				PendingCADeletion: pendingCADeletion,
