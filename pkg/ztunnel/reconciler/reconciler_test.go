...
func TestEnrollmentReconciler_Update(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			endpoints: []*endpoint.Endpoint{
				createTestEndpoint(1, "test-ns", "pod-1", "/var/run/netns/test1"),
				createTestEndpoint(2, "test-ns", "pod-2", ""), // no netns path
			},
			expectedEnrolled: 1,
			expectedError:    false,
		},
...
func TestEnrollmentReconciler_EndpointCreated(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Mark table as initialized and add enrolled namespaces
			txn := db.WriteTxn(tbl)
...
// 	for _, tt := range tests {
// 		t.Run(tt.name, func(t *testing.T) {
// 			db, tbl, mockEpMgr, mockEnroller, _ := setupTest(t)
