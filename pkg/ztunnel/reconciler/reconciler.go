...
func (ops *EnrollmentReconciler) Delete(ctx context.Context, txn statedb.ReadTxn, rev statedb.Revision, ns *Namespace) error {
	...
	if ns.Enrolled {
		...
		for _, ep := range endpoints {
			...
			if err != nil {
				ops.logger.Error("Failed to disenroll endpoint from ztunnel", "namespace", ns.Name, "pod", ep.K8sPodName, "error", err)
				return err
			}
		}
		ops.logger.Info("Disenrolled all endpoints in namespace", "namespace", ns.Name)
	}
	return nil
}
...
func (ops *EnrollmentReconciler) Update(ctx context.Context, txn statedb.ReadTxn, rev statedb.Revision, ns *Namespace) error {
	...
	if ns.PendingEndpointEnrollment {
		...
		for _, ep := range endpoints {
			...
			if err != nil {
				ops.logger.Error("Failed to enroll endpoint to ztunnel", "namespace", ns.Name, "pod", ep.K8sPodName, "error", err)
				return err
			}
		}
		ops.logger.Info("Enrolled all endpoints in namespace", "namespace", ns.Name)
...
func (ops *EnrollmentReconciler) Start(ctx cell.HookContext) error {
	...
	for _, ep := range endpoints {
		...
		if !found || !nsEntry.Enrolled {
			ops.logger.Info("Skipping enrollment of endpoint in unenrolled namespace", "namespace", epNamespace, "pod", ep.K8sPodName)
			continue
		}
	...
	if err != nil {
		ops.logger.Error("Failed to send initial snapshot to ztunnel", "error", err)
		return err
	}
...
func (ops *EnrollmentReconciler) EndpointCreated(ep *endpoint.Endpoint) {
	...
	err := ops.endpointEnroller.EnrollEndpoint(ep)
	if err != nil {
		ops.logger.Error("Failed to enroll endpoint to ztunnel", "namespace", epNamespace, "pod", ep.K8sPodName, "error", err)
	}
}
...
func (ops *EnrollmentReconciler) EndpointDeleted(ep *endpoint.Endpoint, _ endpoint.DeleteConfig) {
	...
	nsEntry, _, found := ops.namespaceTable.Get(txn, EnrolledNamespacesNameIndex.Query(epNamespace))
	if !found || !nsEntry.Enrolled {
		ops.logger.Info("Skipping disenrollment of endpoint in unenrolled namespace", "namespace", epNamespace, "pod", ep.K8sPodName)
		return
	}
	err := ops.endpointEnroller.DisenrollEndpoint(ep)
	if err != nil {
		ops.logger.Error("Failed to disenroll endpoint from ztunnel", "namespace", epNamespace, "pod", ep.K8sPodName, "error", err)
	}
}
