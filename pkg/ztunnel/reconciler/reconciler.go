...
func (ops *EnrollmentReconciler) Start(ctx cell.HookContext) error {
	// Run initialization in a goroutine to avoid blocking the lifecycle.
	// The restorer promise depends on the daemon promise, which depends on
	// k8s cache sync (including policy resources). Blocking here would
	// prevent subsequent lifecycle hooks (e.g., policy watcher) from starting,
	// creating a deadlock.
	go ops.init(context.Background())
	return nil
}
