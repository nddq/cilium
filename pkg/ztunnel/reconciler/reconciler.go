...
func (ops *EnrollmentReconciler) Start(ctx cell.HookContext) error {
	// Start initialization asynchronously to avoid blocking the hive
	// lifecycle. The restorer promise depends on the daemon being fully
	// started, which in turn depends on K8s cache sync completing. If we
	// block here, downstream lifecycle hooks (e.g. the K8s policy watcher)
	// cannot start, creating a circular dependency that prevents the cache
	// sync from ever completing.
	var bgCtx context.Context
	bgCtx, ops.cancel = context.WithCancel(context.Background())
...
func (ops *EnrollmentReconciler) init(ctx context.Context) {
	_, initialized := ops.enrolledNamespaceTable.Initialized(ops.db.ReadTxn())
	select {
	case <-ctx.Done():
		ops.logger.Info("Stopping reconciler before initialization completed")
		return
	case <-initialized:
	}
	...
	if err != nil {
		ops.logger.Error("Failed to await restorer", logfields.Error, err)
		return
	}
	...
	if err := restorer.WaitForEndpointRestore(ctx); err != nil {
		ops.logger.Error("Failed to wait for endpoint restore", logfields.Error, err)
		return
	}
