	...
	for {
		...
		case epEvent := <-sp.endpointRecv:
			if err := sp.handleEPEvent(epEvent); err != nil {
				sp.log.Error("Failed to handle EndpointEvent, closing stream",
					logfields.Error,
					err,
					logfields.EndpointID,
					epEvent.UID)
				// Return to close the stream. When ztunnel reconnects
				// a new resource subscription replays the full state.
				return
			}
		}
	}
