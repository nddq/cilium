...
func (m *MockEndpointEventSource) SubscribeToEndpointEvents(ctx context.Context) {
...
func TestStreamProcessorDeltaDiscoveryRequest(t *testing.T) {
	...
	t.Run("Address Stream Initialization sends empty response and subscribes", func(t *testing.T) {
		var capturedResponse *v3.DeltaDiscoveryResponse
		mockStream := &MockStream{}
		mockStream.OnContext = func() context.Context {
		...
		// Verify subscription was started (runs in a goroutine)
		require.Eventually(t, mockEpSource.GetSubscriptionStatus, time.Second, 10*time.Millisecond,
