...
type Server struct {
	...
	handlerDone chan struct{} // closed when active handler exits; signals pending callers to fail fast
	handlerMu   lock.Mutex    // protects handlerDone
}
...
func newZDSServer(p serverParams) serverOut {
	...
	// handlerDone starts pre-closed so that calls before any handler connects
	// return an error immediately rather than blocking.
	...
	p.Lifecycle.Append(cell.Hook{
		OnStart: func(hc cell.HookContext) error {
			...
			if err := os.MkdirAll(path.Dir(zdsUnixAddr), 0o755); err != nil {
...
func (s *Server) Serve(ctx context.Context) {
	...
	for {
		...
		if cancelPrevConn != nil {
			...
			cancelPrevConn()
			// Wait for the previous goroutine to fully exit before starting
			// the new one. This prevents a race where the old goroutine closes
			// the new handlerDone channel.
			wg.Wait()
		}
		...
		go func() {
			...
			// Signal handler is active by creating a new done channel.
			s.handlerMu.Lock()
			...
			// Signal handler is gone and drain orphaned updates.
			s.handlerMu.Lock()
			close(s.handlerDone)
			s.handlerMu.Unlock()
...
// sendUpdate sends an update to the active handler and waits for it to be processed.
// It returns an error if no handler is active or the handler exits before the update is processed.
func (s *Server) sendUpdate(update zdsUpdate) error {
	s.handlerMu.Lock()
	handlerDone := s.handlerDone
	s.handlerMu.Unlock()

	// Fail fast if no handler is active.
	select {
	case <-handlerDone:
		return fmt.Errorf("no active ztunnel connection")
	default:
	}

	// Send the update, aborting if the handler exits.
	select {
	...
	case err := <-update.errCh:
		...
		return nil
	case <-handlerDone:
		return fmt.Errorf("ztunnel connection lost while waiting for response")
	}
}

// drainPendingUpdates drains the buffered updates channel and sends errors to any waiting callers.
func (s *Server) drainPendingUpdates() {
	for {
		select {
		case update := <-s.updates:
			if update.errCh != nil {
				update.errCh <- fmt.Errorf("ztunnel connection lost")
			}
		default:
			return
		}
...
func (s *Server) EnrollEndpoint(ep *endpoint.Endpoint) error {
	...
	// Add to cache immediately so that sendInitialSnapshot on ztunnel
	// reconnect will enroll this endpoint even if the send below fails
	// (e.g. ztunnel is not yet connected or is restarting).
	...
	if err := s.sendUpdate(update); err != nil {
		...
		// Not fatal: endpoint is cached and will be enrolled when
		// ztunnel reconnects via sendInitialSnapshot.
		s.logger.Warn("failed to enroll endpoint, will retry on ztunnel reconnect",
			logfields.EndpointID, ep.GetID16(),
			logfields.Error, err,
		)
		return nil
